...
class Playlist(object):
    def __init__(self, token, username, playlist_name):

        self.token = token
        self.username = username
        self.playlist_name = playlist_name

        pid, sp = self.get_playlist_id(token, username, playlist_name)

        self.pid = pid
        self.sp = sp

        self.tracks = self.get_playlist_tracks(self.username, self.playlist_id, self.sp)
        self.playlist_uris = [item['track']['uri'] for item in self.tracks]
        self.feature_list = self.get_features(self.tracks, self.sp)
        self.audio_analysis = self.get_playlist_audio_analysis(self.playlist_uris, sp)
